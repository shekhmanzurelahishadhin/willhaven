      ...
      @foreach($languages as $language)
        @if($language->status==1)
            @if($language->language=='en')
                ...
                <form action="{{url('/jobs_updateJobType/'.$thetype->id)}}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="title">Title</label>
                        <input type="text" class="form-control" name="title" value="{{$thetype->title}}" placeholder="Enter job type title">
