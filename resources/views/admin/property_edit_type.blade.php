      ...
      @foreach($languages as $language)
        @if($language->status==1)
            @if($language->language=='en')
                {{-- EN code starts --}}
                <a href="{{route('types')}}" class="btn btn-primary my-3">Return</a>

                <form action="{{url('/property_update_type/'.$eData->id)}}" method="POST">
                    @csrf
            ...
            @elseif($language->language=='gr')
                {{-- GR code starts --}}
                <a href="{{route('types')}}" class="btn btn-primary my-3">Zurückkehren</a>
