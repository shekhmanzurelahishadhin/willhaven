...
<div class="container-fluid" style="width:75vw;">
    <div class="container">
        ...
        <a href="{{route('addDivision')}}" class="btn btn-primary my-3">Create</a>
        @if(Session::has('scc'))
        ...
        <th scope="col" style="width:
        50vw; text-align: center;">Division</th>
        <th scope="col" style="width:
        auto; text-align: center;">Action</th>
        </tr>
        </thead>
        <tbody>
        @foreach ($divisions as $d=>$value)
        <tr>
        <th scope="row" style="text-align: center; font-style: italic;">{{$value->id}}</th>
        <td style="font-size:18px;">{{$value->name}}</td>
        <td style="text-align: center;">
        <a href="{{url('/edit-division/'.$value->id)}}" class="btn btn-success" style="width: 120px;">Edit</a>
        <a href="{{url('/del-division/'.$value->id)}}" onclick = "return confirm('Are you really want to delete?')" class="btn btn-danger" style="width: 120px;">Delete</a>
        </td>
        </tr>
        @endforeach
        </tbody>
