                ...
                @foreach($languages as $language)
                    @if($language->status==1)
                        @if($language->language=='en')
                            <h2 class="text-center my-4">Update Sub Sub Category</h2>
                        @elseif($language->language=='gr')
                            <h2 class="text-center my-4">Unter-Unter-Kategorie aktualisieren</h2>
                        @endif
                    @endif
                @endforeach
                <form class="row g-3" method="POST" action="{{route('update.sub.sub.category')}}">
                    @csrf
                    ...
                    <div class="col-md-12">
                        @foreach($languages as $language)
                            @if($language->status==1)
                                @if($language->language=='en')
                                    ...
                                    <label for="inputCategory" class="form-label">Sub Category</label>
                                @elseif($language->language=='gr')

                                    <label for="inputCategory" class="form-label">Unterkategorie</label>
                                @endif
                            @endif
                        @endforeach
                        <select id="subCategory" name="sub_category_id" class="form-select form-control">
                            ...
                            @foreach($subCategories as $subCategory)
                                <option  value="{{$subCategory->id}}" {{$subCategory->id==$subSubCategory->sub_category_id?'selected':''}}>{{$subCategory->sub_category_name}}</option>
                            @endforeach
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="text" class="form-control" name="sub_sub_category_name" value="{{$subSubCategory->sub_sub_category_name}}" id="inputCategory">
                    </div>
                    <div class="col-12 my-3">
                        @foreach($languages as $language)
                            @if($language->status==1)
                                ...
                                @elseif($language->language=='gr')
                                    <button type="submit" class="btn btn-primary">Unter-Unter-Kategorie aktualisieren</button>
                                @endif
                            @endif
                        @endforeach
