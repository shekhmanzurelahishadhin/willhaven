                ...
                @foreach($languages as $language)
                    @if($language->status==1)
                        @if($language->language=='en')
                            <h2 class="text-center my-4">Add Sub District</h2>
                        @elseif($language->language=='gr')
                            <h2 class="text-center my-4">Unterbezirk hinzufügen</h2>
                        @endif
                    @endif
                @endforeach
                <form class="row g-3" method="POST" action="{{route('save.sub.district')}}">
                    @csrf
                    <div class="col-md-12">
                    ...
                    <div class="col-md-12">
                        ...
                        @foreach($languages as $language)
                            ...
                            @endif
                        @endforeach
                        <input type="text" class="form-control" name="sub_district_name" id="inputCategory">
                    </div>
