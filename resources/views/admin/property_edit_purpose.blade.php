      ...
      @foreach($languages as $language)
        @if($language->status==1)
            @if($language->language=='en')
                {{-- EN code starts --}}
                <a href="{{route('purpose')}}" class="btn btn-primary my-3">Return</a>

                <form action="{{url('/property_update_purpose/'.$eData->id)}}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="">Purpose</label>
                        <input type="text" class="form-control" name="name" value="{{$eData->name}}" placeholder="Enter property type">
            ...
            @elseif($language->language=='gr')
                {{-- GR code starts --}}
                <a href="{{route('purpose')}}" class="btn btn-primary my-3">Zurückkehren</a>
