      ...
      @foreach($languages as $language)
        @if($language->status==1)
            @if($language->language=='en')
                {{-- EN code starts --}}
                <a href="{{route('seller_myProperty')}}" class="btn btn-secondary my-3">Return</a>

                @if(Session::has('scc'))
                      <p class="alert alert-success" style="font-weight:bold;">{{Session::get('scc')}}</p>
                  @endif

                <form action="{{url('/seller_updateProperty/'.$eData->id)}}" method="POST">
                    @csrf
                    <div class="form-group">
                      ...
                      <div class="mb-3">
                        <label for="floor" style="font-size:17px; font-weight:bold;">Total Floor(s):</label>
                        <input type="number" class="form-control" name="floor" value="{{$eData->floor}}" placeholder="Enter total floor" required>
                      </div>
